...
(function(){
 ...
 if(!gaConsent){const banner=document.createElement('div');banner.setAttribute('role','dialog');banner.setAttribute('aria-label','Preferências de privacidade');banner.style.cssText='position:fixed;left:16px;right:16px;bottom:16px;z-index:99999;max-width:680px;margin:auto;padding:18px;background:#211310;color:#fff;border:1px solid #c79b58;border-radius:8px;font:14px/1.5 Arial,sans-serif;box-shadow:0 8px 30px #0008';banner.innerHTML='<strong>Privacidade e cookies</strong><p style="margin:8px 0 14px">Usamos o Google Analytics para entender a visitação e melhorar o site. Você pode aceitar ou recusar cookies de medição.</p><div style="display:flex;gap:10px;flex-wrap:wrap"><button data-ga="accept" style="padding:9px 14px;border:0;border-radius:4px;background:#d6ad68;color:#211310;font-weight:700;cursor:pointer">Aceitar medição</button><button data-ga="reject" style="padding:9px 14px;border:1px solid #d6ad68;border-radius:4px;background:transparent;color:#fff;cursor:pointer">Recusar</button></div>';document.body.appendChild(banner);banner.addEventListener('click',e=>{const choice=e.target.dataset.ga;if(!choice)return;const granted=choice==='accept';localStorage.setItem('cca_analytics_consent',granted?'granted':'denied');gtag('consent','update',{analytics_storage:granted?'granted':'denied'});banner.remove()})}
})();
